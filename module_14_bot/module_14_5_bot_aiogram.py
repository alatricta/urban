...
async def get_buying_list(message: Message, state: FSMContext):
    ...
    for product in cf.get_all_products():
        ptxt = f'Название: {product[1]}\n'\
               f'Описание: {product[2]}\n'\
               f'Цена: {product[3]}'
        await message.answer(text=ptxt)

        file = os.path.normpath(product[4])
...
async def set_age_user(message: Message, state: FSMContext):
    data = await state.update_data(age=message.text)

    cf.add_user(data['username'], data['email'], data['age'])

    await message.answer(text=f'Регистрация пользователя {data["username"]} закончена!')
